...
(function(global) {
	...
	function toPropertyDescriptor(obj) {
		if (Object(obj) !== obj) {
			throw new TypeError(
				'property descriptor should be an Object, given: ' + obj
			);
		}
		var desc = {};
		...
		if ('get' in obj) {
			...
			if (getter !== undefined && typeof getter !== 'function') {
				throw new TypeError(
					"property descriptor 'get' attribute must be " +
						'callable or undefined, given: ' +
						getter
				);
			}
			desc.get = getter;
		...
		if ('set' in obj) {
			...
			if (setter !== undefined && typeof setter !== 'function') {
				throw new TypeError(
					"property descriptor 'set' attribute must be " +
						'callable or undefined, given: ' +
						setter
				);
			}
		...
		if ('get' in desc || 'set' in desc) {
			if ('value' in desc || 'writable' in desc) {
				throw new TypeError(
					'property descriptor cannot be both a data and an ' +
						'accessor descriptor: ' +
						obj
				);
			}
	...
	Validator.prototype = {
		...
		getOwnPropertyDescriptor: function(name) {
			...
			if (desc === undefined) {
				if (isSealedDesc(targetDesc)) {
					throw new TypeError(
						"cannot report non-configurable property '" +
							name +
							"' as non-existent"
					);
				}
				if (!extensible && targetDesc !== undefined) {
					...
					throw new TypeError(
						"cannot report existing own property '" +
							name +
			...
			if (!extensible) {
				if (targetDesc === undefined) {
					throw new TypeError(
						"cannot report a new own property '" +
							name +
			...
			if (name !== undefined) {
				if (!isCompatibleDescriptor(extensible, targetDesc, desc)) {
					throw new TypeError(
						'cannot report incompatible property descriptor ' +
							"for property '" +
							name +
							"'"
					);
			...
			if (desc.configurable === false && !isSealedDesc(targetDesc)) {
				...
				throw new TypeError(
					'cannot report a non-configurable descriptor ' +
						"for configurable or non-existent property '" +
						name +
						"'"
				);
			}
		...
		defineProperty: function(name, desc) {
			...
			if (success === true) {
				...
				if (!extensible) {
					if (targetDesc === undefined) {
						throw new TypeError(
							"cannot successfully add a new property '" +
								name +
				...
				if (targetDesc !== undefined) {
					if (!isCompatibleDescriptor(extensible, targetDesc, desc)) {
						throw new TypeError(
							'cannot define incompatible property ' +
		...
		preventExtensions: function() {
			...
			if (success) {
				if (Object_isExtensible(this.target)) {
					throw new TypeError(
						"can't report extensible object as non-extensible: " + this.target
					);
				}
			}
			return success;
		...
		ownKeys: function() {
			...
			for (var i = 0; i < numProps; i++) {
				...
				if (!Object.isExtensible(this.target) && !isFixed(s, this.target)) {
					...
					throw new TypeError(
						'ownKeys trap cannot list a new ' +
							"property '" +
							s +
							"' on a non-extensible object"
					);
			...
			ownProps.forEach(function(ownProp) {
				if (!propNames[ownProp]) {
					if (isSealed(ownProp, target)) {
						throw new TypeError(
							'ownKeys trap failed to include ' +
					...
					if (!Object.isExtensible(target) && isFixed(ownProp, target)) {
						...
						// non-existent
						throw new TypeError(
							"ownKeys trap cannot report existing own property '" +
								ownProp +
								"' as non-existent on a non-extensible object"
						);
					}
		...
		isExtensible: function() {
			...
			if (result !== state) {
				if (result) {
					throw new TypeError(
						'cannot report non-extensible object as extensible: ' + this.target
					);
				} else {
					throw new TypeError(
						'cannot report extensible object as non-extensible: ' + this.target
					);
				}
			}
			return state;
		...
		has: function(name) {
			...
			if (res === false) {
				if (isSealed(name, this.target)) {
					throw new TypeError(
						'cannot report existing non-configurable own ' +
							"property '" +
							name +
							"' as a non-existent " +
							'property'
    ...
    */
			...
			if (fixedDesc !== undefined) {
				...
				if (
					...
					if (!sameValue(res, fixedDesc.value)) {
						throw new TypeError(
							'cannot report inconsistent value for ' +
					...
					if (
						isAccessorDescriptor(fixedDesc) &&
						fixedDesc.configurable === false &&
						fixedDesc.get === undefined
					) {
						if (res !== undefined) {
							throw new TypeError(
								'must report undefined for non-configurable ' +
									"accessor property '" +
									name +
									"' without getter"
							);
				...
				if (fixedDesc !== undefined) {
					...
					if (
						...
						if (!sameValue(val, fixedDesc.value)) {
							throw new TypeError(
								'cannot successfully assign to a ' +
									"non-writable, non-configurable property '" +
									name +
									"'"
							);
						...
						if (
							...
							throw new TypeError(
								"setting a property '" + name + "' that has " + ' only a getter'
							);
						}
					}
				}
		...
		enumerate: function() {
			...
			if (
				...
				throw new TypeError(
					'enumerate trap should return an iterator, got: ' + trapResult
				);
			}
			...
			while (!nxt.done) {
				...
				if (propNames[s]) {
					throw new TypeError(
						'enumerate trap cannot list a ' + "duplicate property '" + s + "'"
					);
				}
				propNames[s] = true;
				result.push(s);
			...
			ownEnumerableProps.forEach(function(ownEnumerableProp) {
				if (!propNames[ownEnumerableProp]) {
					if (isSealed(ownEnumerableProp, target)) {
						throw new TypeError(
							'enumerate trap failed to include ' +
  ...
  */
	...
	Object.prototype.toString = makeUnwrapping0ArgMethod(
		Object.prototype.toString
	);
	Function.prototype.toString = makeUnwrapping0ArgMethod(
		Function.prototype.toString
	);
	...
	var __proto__setter = (function() {
		var protoDesc = prim_getOwnPropertyDescriptor(
			Object.prototype,
			'__proto__'
		);
	...
	Object.setPrototypeOf = function(target, newProto) {
		...
		if (handler !== undefined) {
			...
			if (!Object_isExtensible(target)) {
				throw new TypeError(
					"can't set prototype on non-extensible object: " + target
				);
			}
			...
			if (Object(newProto) !== newProto || newProto === null) {
				throw new TypeError(
					'Object prototype may only be an Object or null: ' + newProto
				);
	...
	if (typeof Proxy !== 'undefined' && typeof Proxy.create !== 'undefined') {
		...
		global.Proxy = function(target, handler) {
			...
			if (Object(handler) !== handler) {
				throw new TypeError(
					'Proxy handler must be an Object, given ' + handler
				);
			}
		...
		if (typeof Proxy === 'undefined') {
			...
			global.Proxy = function(_target, _handler) {
				throw new Error(
					'proxies not supported on this platform. On v8/node/iojs, make sure to pass the --harmony_proxies flag'
				);
			};
		}
